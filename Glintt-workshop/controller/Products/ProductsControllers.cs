...
using System.Linq;
using AcademiaCodigoWarehouseApi.Controllers.Products;
using Glintt_workshop.controller;
using Glintt_workshop.controller.products;
...
namespace Glintt_workshop.WarehouseApi.Controllers.Products
{
    [Route ("api/products")]
    public class ProductsController : Controller, IProductsEndpoint {
        ...
        public IReadOnlyCollection<ProductSearchItemModel> Search (
            string code, string name, decimal? minPrice,
            decimal? maxPrice, bool? isActive, int skip = 0, int take = 20
        ) {
        ...
        public IActionResult Create ([FromBody] CreateProductModel model) {
            if (ModelState.IsValid) {
                ...
                // abrir uma transacção explícita, mas não é necessário
        ...
        public IActionResult Update (long id, [FromBody] UpdateProductModel model) {
            ...
            if (ModelState.IsValid) {
                var productSet = _ctx.Set<ProductEntity> ();

                var product = productSet.SingleOrDefault (e => e.Id.Equals (id));
                ...
                if (!product.Code.EqualsOrdinalIgnoreCase (modelCode) &&
                    productSet.Any (e => e.Code.EqualsOrdinalIgnoreCase (modelCode))) {
                    return Conflict (new {
                        Message = "Código duplicado"
                    });
                }
                ...
                if (!product.Name.EqualsOrdinalIgnoreCase (modelName) &&
                    productSet.Any (e => e.Name.EqualsOrdinalIgnoreCase (modelName))) {
                    return Conflict (new {
                        Message = "Nome duplicado"
                    });
                }
